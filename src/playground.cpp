...
int main() {
  ...
  auto run_func = [&](const std::string &title, auto f, auto Xlimit,
    ...
    const std::string cmd = "python3 scripts/plot_paths.py " + json_file;
  ...
  run_func("Himmelblau",
           (x * x + y - 11.0) * (x * x + y - 11.0) +
               (x + y * y - 7.0) * (x + y * y - 7.0),
           std::tuple{-5, 5}, std::tuple{-5, 5},
           random_point(-5, 5, -5, 5, rng));

  // Rosenbrock
  // Global min f=0 at (1,1); notoriously difficult curved valley
  run_func(
      "Rosenbrock", (1.0 - x) * (1.0 - x) + 100.0 * (y - x * x) * (y - x * x),
      std::tuple{-2, 2}, std::tuple{-1, 3}, random_point(-2, 2, -1, 3, rng));
  ...
  run_func("Beale", beale, std::tuple{-4.5, 4.5}, std::tuple{-4.5, 4.5},
           random_point(-4.5, 4.5, -4.5, 4.5, rng));

  // Booth
  // Global min f=0 at (1,3); smooth single-minimum quadratic
  auto booth = (x + 2.0 * y - 7.0) * (x + 2.0 * y - 7.0) +
               (2.0 * x + y - 5.0) * (2.0 * x + y - 5.0);
  run_func("Booth", booth, std::tuple{-10, 10}, std::tuple{-10, 10},
           random_point(-10, 10, -10, 10, rng));

  // Matyas
  // Global min f=0 at (0,0); shallow elliptical bowl
  auto matyas = 0.26 * (x * x + y * y) - 0.48 * x * y;
  run_func("Matyas", matyas, std::tuple{-10, 10}, std::tuple{-10, 10},
           random_point(-10, 10, -10, 10, rng));

  // Six-hump camelback
  // Two global minima f≈−1.0316 at ±(0.0898,−0.7126); six local minima
  auto six_hump_camlback = (4.0 - 2.1 * x * x + x * x * x * x / 3.0) * x * x +
                           x * y + (-4.0 + 4.0 * y * y) * y * y;
  run_func("Six-hump camelback", six_hump_camlback, std::tuple{-3, 3},
           std::tuple{-2, 2}, random_point(-3, 3, -2, 2, rng));

  // Goldstein-Price
  // Global min f=3 at (0,−1); four local minima
  {
    auto A = 1.0 + (x + y + 1.0) * (x + y + 1.0) *
                       (19.0 - 14.0 * x + 3.0 * x * x - 14.0 * y + 6.0 * x * y +
                        3.0 * y * y);
    auto B = 30.0 + (2.0 * x - 3.0 * y) * (2.0 * x - 3.0 * y) *
                        (18.0 - 32.0 * x + 12.0 * x * x + 48.0 * y -
                         36.0 * x * y + 27.0 * y * y);
    run_func("Goldstein-Price", A * B, std::tuple{-2, 2}, std::tuple{-2, 2},
             random_point(-2, 2, -2, 2, rng));
  }

  // Rastrigin
  // Many local minima; global min f=0 at (0,0)
  {
    constexpr double pi = std::numbers::pi;
    auto rastrigin = 20.0 + x * x - 10.0 * cos(2.0 * pi * x) +
                     y * y - 10.0 * cos(2.0 * pi * y);
    run_func("Rastrigin", rastrigin, std::tuple{-5.12, 5.12},
             std::tuple{-5.12, 5.12},
             random_point(-5.12, 5.12, -5.12, 5.12, rng));
  }
