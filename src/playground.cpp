...
// ── PathCapture callback ──────────────────────────────────────────────────────
// Records the 2D iterate at each step via on_iter_point.
// Stores a pointer to an external json::array so that value-semantic copies
// (callbacks are passed by value into solvers) still append to the same object.
...
// ── Grid evaluation ───────────────────────────────────────────────────────────
// Evaluates a 2D expression on a regular grid. Returns a JSON object with
// bounds, resolution, and a flat row-major Z array. Python reads this directly
// — it doesn't need to know anything about the C++ expression.
template <typename Algo>
json make_grid(Algo &algo,
               double xlo, double xhi, int nx,
    ...
    return {{"xlo", xlo}, {"xhi", xhi}, {"nx", nx},
            {"ylo", ylo}, {"yhi", yhi}, {"ny", ny},
            {"z",   std::move(z)}};
}
...
int main() {
    ...
    constexpr double xlo = -2.0, xhi = 2.0;
    constexpr double ylo = -1.0, yhi = 3.0;
    constexpr int    nx  = 200,  ny  = 200;

    const Eigen::Vector2d p0{-1.0, 0.5};

    // ── 3. Evaluate grid (once — used for the Python contour) ────────────────
    std::printf("=== %s  start=(%.2f, %.2f) ===\n\n",
                title.c_str(), p0[0], p0[1]);
