...
// Derived must implement (or inherit from NLSTrustRegionBase which provides
// them):
//
//   eval_state(p)                      → tuple<T, ParamVec, NMat>  [phi, g, B]
//   eval_trial(p_new)                  → T   (caches new state internally in
//   Derived) compute_step(g, B, delta)          → ParamVec adjust_tr(delta&,
//   rho, at_bnd)     → void commit_state(step, g_old, B_cur)   → pair<ParamVec,
//   NMat>  [g_new, B_new] refresh_hessian(p, B&)             → void  [default:
//   no-op; Dogleg::ExactAD overrides]
//
