...
TEST(Bracketmethod, QuadraticBrackets) {
  ...
  // After bracketing, bx must be strictly between ax and cx with f(bx) < f(ax)
  // and f(bx) < f(cx)
...
TEST(Golden, NegativeQuadratic) {
  // f(x) = -(x - 1)^2 + 4  =>  this is a maximum, minimum is at the bracket
  // edges. Instead test f(x) = (x + 1)^2, minimum at x = -1
  auto x = diff::Variable<double, 'x'>{0.0};
  auto f = (x + 1.0) * (x + 1.0);
...
TEST(Brent, QuarticMinimum) {
  // f(x) = (x-1)^4  — flat near minimum, good stress test for parabolic
  // interpolation
  auto x = diff::Variable<double, 'x'>{0.0};
...
TEST(LinMin, AxisDirection) {
  ...
  EXPECT_NEAR(lm.get_optimal_value(), 16.0, kTol); // (3-3)^2 + (0-4)^2 = 16
}
...
TEST(LinMin, DiagonalDirection) {
  ...
  EXPECT_NEAR(lm.get_optimal_value(), 0.5, kTol); // (3.5-3)^2 + (3.5-4)^2 = 0.5
}
...
TEST(Frprmn, Bowl2D) {
  ...
  EXPECT_NEAR(p[0], 1.0, kTol);
  EXPECT_NEAR(p[1], 2.0, kTol);
  EXPECT_NEAR(cg.get_optimal_value(), 0.0, kTol * kTol);
}
...
TEST(Frprmn, Rosenbrock) {
  ...
  EXPECT_NEAR(p[0], 1.0, 1e-4);
  EXPECT_NEAR(p[1], 1.0, 1e-4);
  EXPECT_NEAR(cg.get_optimal_value(), 0.0, 1e-6);
}
...
TEST(BFGS, Bowl2D) {
  ...
  EXPECT_NEAR(p[0], 1.0, kTol);
  EXPECT_NEAR(p[1], 2.0, kTol);
  EXPECT_NEAR(bfgs.get_optimal_value(), 0.0, kTol * kTol);
}
...
TEST(BFGS, Rosenbrock) {
  ...
  EXPECT_NEAR(p[0], 1.0, 1e-4);
  EXPECT_NEAR(p[1], 1.0, 1e-4);
  EXPECT_NEAR(bfgs.get_optimal_value(), 0.0, 1e-6);
}
...
TEST(AugLag, InequalityConstrained) {
  ...
  // Unconstrained min at (3,3) violates x+y≤4; active-constraint min at x=y=2,
  // f=2
  auto x = diff::Variable<double, 'x'>{0.0};
  auto y = diff::Variable<double, 'y'>{0.0};
...
TEST(LBFGS, Rosenbrock) {
  ...
  EXPECT_NEAR(p[0], 1.0, 1e-4);
  EXPECT_NEAR(p[1], 1.0, 1e-4);
  EXPECT_NEAR(lbfgs.get_optimal_value(), 0.0, 1e-6);
}
